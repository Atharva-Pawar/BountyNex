            ...
            <>
              <div className="flex items-center justify-between rounded-lg border border-border bg-surface-2 px-4 py-3">
                <span className="flex items-center gap-2 text-sm text-ink-dim">
                  <Wallet2 className="h-4 w-4 text-accent" /> {shortAddress(wallet.address, 6)}
                </span>
        ...
        <Card>
          ...
          <CardBody>
            <ul className="space-y-2">
              {txData.items.map((tx) => (
                <li key={tx.id} className="flex items-center justify-between rounded-lg border border-border bg-surface-2 px-4 py-2.5">
                  <div>
